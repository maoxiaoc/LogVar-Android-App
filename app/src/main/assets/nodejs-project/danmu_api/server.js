...
function createServer() {
  ...
  return http.createServer(async (req, res) => {
    if (process.env.LOGVAR_ANDROID === '1' && req.url === '/_logvar/health' &&
        ['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(req.socket.remoteAddress)) {
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ ready: true, version: Globals.VERSION }));
      return;
    }
    const limited = process.env.LOGVAR_ANDROID === '1' && /\/api\/v2\/comment\//.test(req.url);
    if (limited && activeComments >= 2) {
      res.writeHead(503, { 'Retry-After': '2', 'Content-Type': 'text/plain; charset=utf-8' });
      res.end('弹幕服务繁忙，请稍后重试');
      return;
    }
    if (limited) activeComments++;
    try {
      ...
      if (acceptEncoding.includes('gzip') && buffer.length > 1024 &&
          ...
          try {
              const compressed = await new Promise((resolve, reject) =>
                  zlib.gzip(buffer, (error, output) => error ? reject(error) : resolve(output)));
...
async function startServer() {
  if (mainServer?.listening) return;
...
// The Android shell keeps the Node runtime alive. These controls only close or
// reopen sockets; they never terminate the embedded runtime.
async function stopServerForApp() {
  stopFavoriteScheduler();
  if (envWatcher) { await envWatcher.close(); envWatcher = null; }
  await Promise.all([mainServer, proxyServer].filter(server => server?.listening).map(server => new Promise(resolve => server.close(resolve))));
}
globalThis.logvarStartServer = startServer;
globalThis.logvarStopServer = stopServerForApp;
