...
export default class BilibiliSource extends BaseSource {
    ...
    for (let i = 0; i < segmentList.length; i += BATCH_SIZE) {
        ...
        for (const res of results) {
            if (res.status === 'ok' && res.value) {
                ...
                if (res.error?.bilibiliCode !== undefined) throw res.error;
                // 请求失败视为视频结束（熔断机制）
  ...
  async getEpisodeSegmentDanmu(segment) {
    try {
      ...
      // HTTP 200 can contain an API error instead of protobuf; never cache it as empty.
      const contentType = response?.headers?.['content-type'] || response?.headers?.['Content-Type'] || '';
      if (contentType.includes('application/json') && response.data) {
        const result = JSON.parse(atob(response.data));
        if (result.code !== 0) {
          const error = new Error(`[bilibili] 弹幕接口错误: ${result.code}`);
          error.bilibiliCode = result.code;
          throw error;
        }
      }
