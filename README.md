...
API Token 用于播放器访问；管理 Token 用于管理功能，不应填写到播放器或公开分享。当前 API 使用 HTTP，适合可信的本机或局域网环境，请勿直接暴露到公网。反馈问题时隐藏完整 API 地址、Token、Cookie 和个人日志信息。
...
普通问题请提交 [Issue](https://github.com/maoxiaoc/LogVar-Android-App/issues)，附应用版本、Android 版本、设备架构、复现步骤和脱敏后的错误信息。请勿上传签名文件、完整 Token、Cookie 或个人私密数据。安全问题请参考 [SECURITY.md](SECURITY.md)。

若涉及名称、图标或其他权利问题，请提供具体材料及权利依据，维护者会核实处理。
